...
export function PerformanceChart() {
  ...
  const [timeframe, setTimeframe] = React.useState<'all' | 'month' | 'week' | 'day'>('all');
  const [variant, setVariant] = React.useState<string>('all');

  // Filter chartData based on timeframe/variant
  const filteredData = React.useMemo(() => {
    // You can add your filtering logic here based on timeframe/variant
    ...
    <Card className="py-4 sm:py-0">
      <CardHeader className="flex flex-col items-stretch border-b !p-0 sm:flex-row">
        <div className="flex flex-1 flex-col justify-center gap-1 px-6 pb-3 sm:pb-0">
          ...
          <CardDescription>
            Showing total performance for the selected period
          </CardDescription>
        </div>
        <div className="flex">
      ...
      <div className="flex gap-4 px-6 py-2">
        ...
        <div>
          ...
          >
            <option value="all">All</option>
            {/* Add more options based on timeframe if needed */}
          </select>
        </div>
      </div>
